...
server <- function(input, output) {
    ...
    output$distPlot1 <- renderPlot({
        ...
        abline(v=c(days_vector/365), col="green", lty=2,lwd = 2)
        legend("bottomright", c("Dogs", "Bugs"), pch = 16, bty = "n", col = c("blue","red"))
    ...
    output$distPlot2 <- renderPlot({
        ...
        ###### Run the ode solver function
        out <- as.data.frame(ode(y = y0, times = times, func = SetODEs, parms = parameters))
        RESULTS<-data.frame(out$m,out$Y*out$m)
        ...
        abline(v=c(days_vector/365), col="green", lty=2, lwd =2)
        legend("bottomright", c("Bugs per Dog", "Infected Bug per Dog"), pch = 16, bty= "n", col = c("blue","red"))
