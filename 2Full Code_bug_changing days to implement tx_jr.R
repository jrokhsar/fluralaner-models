...
#       Trying to assign the treatment to start at 10000 days 
#       jr added vector "time" to be used in the signal$import
#       test both dataframes to implement treatment at 10000 days 
#           signal: 
#=============================================

#create two time vectors
times <- seq(0, 20000, by = 1)
time <- seq(0, 10001, by=1)


#Create two empty dataframes to test the codes to implement treatment at 10000 days
signal <- data.frame(times = times, import = rep(0, length(times)))
signal2 <- data.frame(times = times, import2 = rep(0, length(times)))


#two different codes to test treatment implementation at 10000 days
signal2$import2[c(10000:20001)] = (Asym / (1 + exp((xmid - times[10000:20001]) / scal)))  
signal$import[c(10000:20001)] = (Asym / (1 + exp((xmid - time) / scal)))  


#Graph the two codes for signal import.
par(mfrow=c(1,2))

(plot(times,signal$import, xlim = c(9900, 11000),
     main="(Asym / (1 + exp((xmid - times)", cex.main=0.8, cex=0.5))

(plot(times,signal2$import2, xlim = c(9900, 11000),
      main="(Asym / (1 + exp((xmid - times[10000:20001])", cex.main=0.8, cex=0.5))
...
#=============================================
par(mfrow=c(1,4))
plot(outTx2$m, type='l',main='Bugs per Dog')
plot(outTx2$X, type='l', main= 'Dog Prevalence')
plot(outTx2$Y*outTx2$m, type='l', main = 'Positive Bugs Per Dog')
plot(outTx2$Y, type='l', main = 'Bug prevalence')
